...
public class VladDogAccessList {

  private static VladDogAccessList dfltInstance;  private Database dbAlias;
  ...
  public VladDogAccessList(Database db) {
    try {
      dbAlias = db;
      jbInit();
    } catch (Exception e) {
  ...
  public static VladDogAccessList getInstance(){ 
    if (dfltInstance==null) dfltInstance = new VladDogAccessList(); 
    return dfltInstance; 
  } 
...
//    pRowSelect.addColumn("ID", Variant.INT);
    ...
    pdsVLAD_DOG.setProcedure(new ProcedureDescriptor(dbAlias, "exec lsp_VLAD_DOG_S_v1", pRowSelect, true, Load.ALL));
